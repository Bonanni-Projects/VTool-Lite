...
if ~isempty(Selections)
  ...
  for k = 1:length(Stats)
    Stats(k).LtMin  = structfun(@(x)x(:,index),Stats(k).LtMin, 'Uniform',false);
    Stats(k).LtMax  = structfun(@(x)x(:,index),Stats(k).LtMax, 'Uniform',false);
...
tag = sprintf('PlotLtStats: %s', datestr(now));
...
for m = 1:M
  ...
  if strcmp(PlotMode,'normal')
    subplot(3,3,3*(row-1)+1)
  else  % if strcmp(PlotMode,'single')
    figure, set(gcf,'Name',[name,' - min'],'NumberTitle','off')
  end
  C50 = arrayfun(@(x)x.LtMin.p50(:,m),Stats,'Uniform',false);
  C95 = arrayfun(@(x)x.LtMin.p95(:,m),Stats,'Uniform',false);
  C05 = arrayfun(@(x)x.LtMin.p05(:,m),Stats,'Uniform',false);
  Y   =     cat(2,C50{:});
  POS =     cat(2,C95{:}) - Y;
  NEG = Y - cat(2,C05{:});
  for k = 1:size(Y,2)
  ...
  if strcmp(PlotMode,'normal')
    subplot(3,3,3*(row-1)+2)
  else  % if strcmp(PlotMode,'single')
    figure, set(gcf,'Name',[name,' - mean'],'NumberTitle','off')
  end
  ...
  if strcmp(PlotMode,'normal')
    subplot(3,3,3*(row-1)+3)
  else  % if strcmp(PlotMode,'single')
    figure, set(gcf,'Name',[name,' - max'],'NumberTitle','off')
  end
  C50 = arrayfun(@(x)x.LtMax.p50(:,m),Stats,'Uniform',false);
  C95 = arrayfun(@(x)x.LtMax.p95(:,m),Stats,'Uniform',false);
  C05 = arrayfun(@(x)x.LtMax.p05(:,m),Stats,'Uniform',false);
  Y   =     cat(2,C50{:});
  POS =     cat(2,C95{:}) - Y;
  NEG = Y - cat(2,C05{:});
  for k = 1:size(Y,2)
  ...
  if (row == 3 || m == M) && strcmp(PlotMode,'normal')
    ...
    titlestr = sprintf('%s     -  LT statistics',strrep(figname,'_','\_'));
    suptitle(titlestr)
