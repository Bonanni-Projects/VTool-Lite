...
if ~isempty(vars)
  % Check pairs
  OptionsList = {'CategoryFun','DownSampFactor','NanSeparators','OutputTag'};
...
% ------------------------------------------------------------------------------
function results = ProcessFiles(pathnames,datafun,downsampfactor,nanseparators)

% Process the list of input files defined by 'pathnames', with 
% dataset function 'datafun', downsampling factor 'downsampfactor', 
% and separation option 'nanseparators'.  Return collected quantities 
% as fields of 'results' structure. 
...
for k = 1:ncases
  ...
  Data1 = datafun(pathname);

  % Downsample according to 'downsampfactor'
  Data1 = DownsampleDataset(Data1,downsampfactor);

  % If NaN separation is specified
  if strcmp(nanseparators,'on')
    Data1 = ApplyMask(Data1,'last',nan);
  end
  ...
  if k == 1
    ...
    Data = ConcatDatasets(Data,Data1);
  end
end
