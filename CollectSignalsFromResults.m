...
elseif nargin > 2
  ...
  OptionsList = {'Selections','LoadFun','CategoryFun','DownSampFactor','TimeRange','OutputTag'};
...
% ----------------------------------------------------------------------------------------
function results = ProcessResultsFiles(pathnames,loadfun,downsampfactor,timerange,names)
...
% collected/concatenated variables as fields of 'results' structure. 
...
for k = 1:ncases
  ...
  for j = 1:length(vnames)
    ...
    Tname = sprintf('TIMES_%s',vname);    % Time group
    ...
    if isdataset(j)  % if variable is a dataset
      ...
      s.(vname) = LimitTimeRange(s.(vname),timerange);          % limit time range according to 'timerange'
...
% Check each 'TIMES' array to see if all entries are equal.
% If so, reduce it to a scalar signal group called 'Time_<...>'.
for j = 1:length(vnames)
  vname = vnames{j};   % variable name
  if IsSignalGroup(S(1).(vname),'Time')
    if length(S) > 1 && isequal(S.(vname))
      vname1 = strrep(vname,'TIMES','Time');
      S(1).(vname1) = S(1).(vname);
      S = rmfield(S,vname);
    end
  end
end
