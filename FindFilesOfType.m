function [pathnames,fnames] = FindFilesOfType(dname,filetype,depth)
...
if iscell(dname)
  ...
  for k = 1:length(dnames)
    ...
    fprintf('----------------------------------------------------------------------------------------------------------------\n');
    [pathnames1,fnames1] = FindFilesOfType(dnames{k},filetype,depth);
...
% Check 'dname' and 'filetype' inputs
if ~ischar(dname)
...
if ~isempty(incfile)
  ...
  mask = ~IsFileType(inclusions,filetype) | ...
...
if ~isempty(excfile)
  ...
  mask = ~IsFileType(exclusions,filetype) | ...
